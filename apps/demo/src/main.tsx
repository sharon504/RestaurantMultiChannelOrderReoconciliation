...
function App() {
  ...
  useEffect(() => { loadDashboard().then(({ data, source: loadedFrom }) => { setDashboard(data); setSource(loadedFrom); }); }, []);
  useEffect(() => {
  ...
  return <>
    ...
    <main id="overview" className="shell">
      <section className="hero reveal"><div><p className="eyebrow">Daily close / {dashboard.businessDate}</p><h1>One close. Every channel accounted for.</h1><p className="lede">Review the evidence behind today’s revenue figure, then investigate only the records that do not agree.</p></div><div className="close-panel"><div className="panel-top"><span>close snapshot</span><span className="status status-closed">immutable</span></div><strong>{money(dashboard.close.revenue)}</strong><div className="close-meta"><span>{dashboard.close.orderCount} orders</span><span>{dashboard.close.closedAt}</span></div><code>close_id: {dashboard.close.id}</code></div></section>
    ...
    <footer>Demo UI · Read-only by design · data source: {source === "api" ? "configured API" : "local fixture"}</footer>
