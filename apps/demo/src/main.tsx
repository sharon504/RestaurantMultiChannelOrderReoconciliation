  ...
  return <>
    ...
    <main id="overview" className="shell">
      ...
      <section className="simulator reveal" aria-labelledby="simulator-title">
        <div className="simulator-heading"><div><p className="eyebrow">Scenario lab</p><h2 id="simulator-title">Run the close, then introduce late evidence.</h2><p>Each action mutates the shared D1-backed demo state. Reset restores the original fixture so you can repeat the sequence.</p></div><span className={`status ${canSimulate ? "status-closed" : "review"}`}>{canSimulate ? "live sandbox" : "API unavailable"}</span></div>
        ...
        <p className="simulation-message" aria-live="polite">{simulationMessage ?? (canSimulate ? "Suggested path: reset → close day → apply settlement." : "Start the D1-backed Worker to enable simulation controls.")}</p>
      </section>
    ...
    <footer>Demo UI · D1-backed simulation sandbox · data source: {source === "api" ? "configured API" : source === "fallback" ? `local fixture (API fallback${loadMessage ? `: ${loadMessage}` : ""})` : "local fixture"}</footer>
