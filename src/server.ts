...
function seedBaseline() {
  ...
  service.ingestKitchen(
    fixture.kitchen.map(([externalId, orderRef]) => ({
      source: "kitchen",
      externalId,
      orderRef,
      cookedAt: `${fixture.date}T18:00:00.000Z`,
    })),
  );
}
...
function serveStatic(url: URL, res: any) {
  ...
  if (!existsSync(file))
    return send(res, 404, { error: "frontend build not found" });
...
createServer(async (req: any, res: any) => {
  ...
  try {
    if (req.method === "GET" && url.pathname === "/api/state")
      return send(res, 200, service.list());
    if (req.method === "GET" && url.pathname === "/api/reconciliation")
      return send(res, 200, service.reconcile(date));
    if (req.method === "GET" && url.pathname === "/api/exceptions")
      return send(
        res,
        200,
        service.list().exceptions.filter(
          (item) => item.date === date && !item.resolvedAt,
        ),
      );
    if (req.method === "GET" && url.pathname === "/api/closes")
      return send(res, 200, service.list().closes);
    if (req.method === "GET" && url.pathname === "/api/adjustments")
      return send(res, 200, service.list().adjustments);
    if (req.method === "POST" && url.pathname === "/api/close")
      return send(res, 200, service.close(date));
    if (req.method === "POST" && url.pathname === "/api/adjust") {
      ...
      if (service.list().settlements.length === 0)
        service.ingestSettlements(fixture.settlements);
      return send(res, 200, service.adjust());
    }
    if (req.method === "POST" && url.pathname === "/api/reset") {
      resetBaseline();
      return send(res, 200, { ok: true, state: service.list() });
    }
    if (req.method === "POST" && url.pathname === "/api/ingest/orders")
      return send(
        res,
        200,
        service.ingestOrders(
          rows<Omit<Order, "id" | "ingestedAt">>(await readJson(req)),
        ),
      );
    if (req.method === "POST" && url.pathname === "/api/ingest/kitchen")
      return send(
        res,
        200,
        service.ingestKitchen(
          rows<Omit<Kitchen, "id" | "ingestedAt">>(await readJson(req)),
        ),
      );
    if (req.method === "POST" && url.pathname === "/api/ingest/settlements")
      return send(
        res,
        200,
        service.ingestSettlements(
          rows<Omit<Settlement, "id">>(await readJson(req)),
        ),
      );
