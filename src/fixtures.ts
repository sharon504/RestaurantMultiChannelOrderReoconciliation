...
export const fixture={date,orders:[
 ...
 settlements:[{source:'agg1',externalId:'S-300',orderRef:'G1-300',businessDate:date,settled:1500,commission:250,platformDiscount:200,receivedAt:'2026-08-12T09:00:00.000Z'},{source:'agg2',externalId:'S-400',orderRef:'G2-400',businessDate:date,settled:1530,commission:270,platformDiscount:0,receivedAt:'2026-08-12T09:00:00.000Z'}]};
export function writeFixtures(){mkdirSync('data',{recursive:true});writeFileSync('data/orders.json',JSON.stringify(fixture.orders,null,2));writeFileSync('data/kitchen.json',JSON.stringify(fixture.kitchen.map(([externalId,orderRef])=>({source:'kitchen',externalId,orderRef,cookedAt:`${date}T18:00:00.000Z`})),null,2));writeFileSync('data/settlements.json',JSON.stringify(fixture.settlements,null,2));writeFileSync('data/ground-truth.json',JSON.stringify({date,exceptionReasons:['COOKED_UNPAID','PAID_UNCOOKED','CANCELLED_AFTER_COOKING','COMMISSION_MISMATCH','AMBIGUOUS_DUPLICATE'],adjustmentAmount:50},null,2));}
