...
Render runs the full Node reconciliation engine and serves the built React demo from the same service, so the frontend uses the real API at the same origin. The service seeds the baseline order and kitchen fixture on first start. `POST /api/reset` restores that baseline; `POST /api/adjust` introduces the bundled T+2 settlement into the demo, while production callers can ingest settlement rows through `POST /api/ingest/settlements`.

Create a Render Web Service from this repository using [render.yaml](render.yaml). It builds the backend and demo UI, starts `node dist/server.js`, and health-checks `/health`. No Cloudflare Worker or D1 database is required.
